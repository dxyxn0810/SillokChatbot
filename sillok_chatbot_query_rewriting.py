...
class SillokInterviewBot:
    ...
    def rewrite_query(self, question: str) -> str:
        """
        사용자 질문을 조선시대 실록 검색에 최적화하도록 1회 재작성.
        - 현재 시기 맥락(왕, 연도, 월) 통합
        - 현대 표현 → 고문 표현 변환 유도
        """
        rewrite_prompt = ChatPromptTemplate.from_template("""
당신은 조선시대 실록 검색 전문가입니다.

현재 맥락:
- 왕: {king}
- 시기: {year} {month}

사용자의 질문을 "조선시대 실록 문서"에서 검색하기에 최적화하도록 재작성하세요.
- 현대 표현을 고어/문어로 자연스럽게 변환
- 왕과 시기 정보를 자연스럽게 포함
- 검색 효율을 높이도록 간결하게
...
최적화된 검색 질문 (1줄):
    ...
    def ask(self, question: str) -> str:
        # 1-1) 원본 질문으로 검색
        docs_original = self.vs.similarity_search(question, k=self.k)
        
        # 1-2) 재작성된 질문으로 검색 (1회)
        try:
            rewritten_question = self.rewrite_query(question)
            docs_rewritten = self.vs.similarity_search(rewritten_question, k=self.k)
        except Exception as e:
            print(f"⚠️  쿼리 재작성 실패: {e} → 원본만 사용합니다.")
            docs_rewritten = []
        
        # 1-3) 검색 결과 병합 (중복 제거, 스코어 합산)
        # Document는 page_content로 동일성 판단
        doc_map = {}  # page_content → (doc, score)
        
        for doc in docs_original:
            content_key = doc.page_content[:100]  # 처음 100자로 고유 키 생성
            if content_key not in doc_map:
                doc_map[content_key] = (doc, 0)
            doc_map[content_key] = (doc_map[content_key][0], doc_map[content_key][1] + 1)
        
        for doc in docs_rewritten:
        ...
        # 스코어순 정렬 후 상위 k개 선택
        sorted_results = sorted(doc_map.values(), key=lambda x: x[1], reverse=True)
        docs = [d[0] for d in sorted_results[:self.k]]
